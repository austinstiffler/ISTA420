...
--create CTE here to alias the "end of year"
...
select o.empid, max(o.orderdate) as maxorderdate from sales.orders o group by o.empid;
...
inner join (select o.empid, max(o.orderdate) as maxorderdate from sales.orders o group by o.empid) as o2
...
drop view if exists dbo.austin;
go
create view dbo.austin as
...
go
select * from dbo.austin order by empid, orderyear;
go
...
-- Write a query against Sales.VEmpOrders (renamed austin)
-- that returns the running qty for each employee and year
-- Tables involved: TSQLV4 database, Sales.VEmpOrders view

select sum(x.qty) from dbo.austin x;

select * from dbo.austin order by empid, orderyear;
--------------------------------------------------
select sum(a1.qty) as runqty from dbo.austin a1 where a1.empid = 1
and a1.orderyear <= 2016
--------------------------------------------------
select a.empid, a.orderyear, a.qty, 
(select sum(a1.qty) as runqty from dbo.austin a1 where a.empid = a1.empid
and a1.orderyear <= a.orderyear) as runqty
  from austin a
  order by a.empid, a.orderyear;
